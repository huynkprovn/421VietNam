	...
	internal class Lucian : Champion
	{
		...
		private void Cast_Q(bool mode)
		{
			...
			if (mode)
			{
				...
				target = LeagueSharp.Common.TargetSelector.GetTarget(Q2.Range, LeagueSharp.Common.TargetSelector.DamageType.Physical);
				if(target == null)
					return;
		...
		private void Cast_W()
		{
			if(!W.IsReady() || !_canUseSpells)
				return;
			var target = LeagueSharp.Common.TargetSelector.GetTarget(W.Range, LeagueSharp.Common.TargetSelector.DamageType.Magical);
			if(target == null)
				return;
		...
		private void Cast_E(bool mode)
		{
			...
			if (mode)
			{
				var target = LeagueSharp.Common.TargetSelector.GetTarget(1100, LeagueSharp.Common.TargetSelector.DamageType.Physical);
				if (target == null)
					return;
				_passivTimer = Environment.TickCount;
			...
			switch(mode)
			{
				case 1:
					if(Q.IsReady() || W.IsReady() || E.IsReady())
						return;
					var target = LeagueSharp.Common.TargetSelector.GetTarget(R.Range, LeagueSharp.Common.TargetSelector.DamageType.Physical);
					...
					break;
				case 2:
					target = LeagueSharp.Common.TargetSelector.GetTarget(R.Range, LeagueSharp.Common.TargetSelector.DamageType.Physical);
