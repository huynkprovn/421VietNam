...
namespace Ultimate_Carry_Prevolution.Plugin
{
	class Gnar : Champion
	{
		...
		private void Cast_Q(bool mode)
		{
			...
			if (mode)
			{
				var target = LeagueSharp.Common.TargetSelector.GetTarget(tempSpell.Range, LeagueSharp.Common.TargetSelector.DamageType.Physical);
				if(target == null)
					return;
		...
		private void Cast_W()
		{
			if(!W.IsReady())
				return;
				var target = LeagueSharp.Common.TargetSelector.GetTarget(W.Range,LeagueSharp.Common.TargetSelector.DamageType.Physical);
				if(target == null)
					return;
		...
		private void Cast_E()
		{
			...
			var target = LeagueSharp.Common.TargetSelector.GetTarget(tempSpell.Range + tempSpell.Width / 2, LeagueSharp.Common.TargetSelector.DamageType.Physical);
			if(target == null)
				return;
		...
		private void Cast_R()
		{
			...
			{
				var target = LeagueSharp.Common.TargetSelector.GetTarget(390, LeagueSharp.Common.TargetSelector.DamageType.Physical);
