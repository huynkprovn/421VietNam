...
namespace Ultimate_Carry_Prevolution.Plugin
{
	class Ahri : Champion 
	{
		...
		{
			if(IsSpellActive("E"))
				Cast_BasicSkillshot_Enemy(E, LeagueSharp.Common.TargetSelector.DamageType.Magical);
			if(IsSpellActive("Q"))
				Cast_BasicSkillshot_Enemy(Q,LeagueSharp.Common.TargetSelector.DamageType.Magical);
			if(IsSpellActive("W"))
				Cast_W();
			if(IsSpellActive("R"))
				Cast_R();
