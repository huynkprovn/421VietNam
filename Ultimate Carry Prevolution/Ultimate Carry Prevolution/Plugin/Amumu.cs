	...
	class Amumu : Champion
	{
		...
		private void WTurnOffCheck()
		{
			if(MyHero.Spellbook.GetSpell(SpellSlot.W).ToggleState != 2 || !W.IsReady())
				return;
			var target = LeagueSharp.Common.TargetSelector.GetTarget(W.Range + Menu.Item("Misc_useW_turnOff").GetValue<Slider>().Value, LeagueSharp.Common.TargetSelector.DamageType.Magical);
			var target2 = LeagueSharp.Common.TargetSelector.GetTarget(W.Range, LeagueSharp.Common.TargetSelector.DamageType.Magical);
		...
		private void Cast_Q(int mode, bool mode2)
		{
			...
			if(mode2)
			{
				...
				if (! R.IsReady( ))
				{
					target = LeagueSharp.Common.TargetSelector.GetTarget(Q.Range, LeagueSharp.Common.TargetSelector.DamageType.Magical);
					if(target == null)
						return;
