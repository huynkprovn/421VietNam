	...
	class Alistar : Champion
	{
		...
		private void Cast_QW_Combo()
		{
			...
			if(_moveOrder)
			{
				...
				var reached = Runto(GetMovePosition(LeagueSharp.Common.TargetSelector.GetTarget(Q.Range, LeagueSharp.Common.TargetSelector.DamageType.Magical)));
				if(reached)
				{
					W.Cast(LeagueSharp.Common.TargetSelector.GetTarget(Q.Range, LeagueSharp.Common.TargetSelector.DamageType.Magical),UsePackets());
					return;
				}
			}
			else
