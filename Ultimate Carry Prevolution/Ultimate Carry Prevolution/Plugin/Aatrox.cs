...
namespace Ultimate_Carry_Prevolution.Plugin
{
	class Aatrox : Champion
	{
		...
		private void Cast_Q()
		{
			var target = LeagueSharp.Common.TargetSelector.GetTarget(Q.Range + Q.Width / 2,LeagueSharp.Common.TargetSelector.DamageType.Physical);
			switch(xSLxOrbwalker.CurrentMode)
			{
				case xSLxOrbwalker.Mode.Combo:
					if(target != null && !xSLxOrbwalker.InAutoAttackRange(target ))
						Cast_BasicSkillshot_Enemy(Q, LeagueSharp.Common.TargetSelector.DamageType.Physical);
					break;
		...
		private void Cast_E()
		{
			var target = LeagueSharp.Common.TargetSelector.GetTarget(E.Range, LeagueSharp.Common.TargetSelector.DamageType.Physical);
			if(target != null)
				Cast_BasicSkillshot_Enemy(E,LeagueSharp.Common.TargetSelector.DamageType.Physical);
		}
