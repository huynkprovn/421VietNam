...
namespace Ultimate_Carry_Prevolution
{
	class Champion
	{
		...
		public Obj_AI_Hero Cast_BasicSkillshot_Enemy(Spell spell, LeagueSharp.Common.TargetSelector.DamageType prio = LeagueSharp.Common.TargetSelector.DamageType.True, float extrarange = 0)
		{
			if(!spell.IsReady())
				return null;
			var target = LeagueSharp.Common.TargetSelector.GetTarget(spell.Range + extrarange, prio);
			if(target == null)
				return null;
